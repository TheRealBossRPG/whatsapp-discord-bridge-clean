...
class Instance {
  ...
  constructor(options) {
    ...
    setTimeout(() => {
      ...
      if (this.isConnected()) {
        console.log(
          `[Instance:${this.instanceId}] Running initial connection health check`
        );
        this.verifyActiveConnection().then((isReallyConnected) => {
          if (!isReallyConnected) {
            console.log(
              `[Instance:${this.instanceId}] Initial health check shows connection issues, scheduling refresh`
            );
            // Schedule a refresh in 1 minute
            setTimeout(() => {
              if (this.isConnected() && !this.reconnecting) {
                console.log(
                  `[Instance:${this.instanceId}] Performing initial connection refresh`
                );
            ...
            console.log(
              `[Instance:${this.instanceId}] Initial health check passed, connection is working properly`
            );
          }
        });
      }
  ...
  async initializeMentionProcessor() {
    try {
      ...
      console.error(
        `[Instance:${this.instanceId}] Error initializing MentionProcessor:`,
        error
      );
      return false;
    }
  ...
  /**
   * Load service status from settings
   */
  async loadServiceStatus() {
    try {
      ...
      if (fs.existsSync(settingsPath)) {
        ...
        // Check if service is marked inactive
        if (settings.status === "inactive") {
          console.log(
            `[Instance:${this.instanceId}] This instance is marked as INACTIVE`
          );
          this.serviceActive = false;

          // Stop connection refresh timer if it's running
      ...
      console.error(
        `[Instance:${this.instanceId}] Error loading service status:`,
        error
      );
    }
  }
  ...
  async initializeClient() {
    try {
      ...
      if (!this.clients.whatsAppClient) {
        console.log(
          `[Instance:${this.instanceId}] Creating new WhatsApp client...`
        );
        ...
        console.log(
          `[Instance:${this.instanceId}] WhatsApp client created successfully`
        );
      } else {
        console.log(
          `[Instance:${this.instanceId}] WhatsApp client already exists`
        );
      }
      ...
      console.error(
        `[Instance:${this.instanceId}] Error initializing WhatsApp client:`,
        error
      );
      return false;
    }
  ...
  async initializeHandlers() {
    ...
    if (this.vouchChannelId) {
      ...
      if (
        this.customSettings &&
        this.customSettings.hasOwnProperty("vouchEnabled")
      ) {
        this.handlers.vouchHandler.isDisabled =
          !this.customSettings.vouchEnabled;
      }
    }
  ...
  startConnectionRefreshTimer() {
    ...
    if (!this.serviceActive) {
      console.log(`[Instance:${this.instanceId}] Not starting connection refresh timer - service is inactive`);
      return;
    }

    console.log(
      `[Instance:${this.instanceId}] Starting connection refresh timer (every ${
        this.refreshIntervalMs / (60 * 60 * 1000)
      } hours)`
    );
    ...
    this.connectionRefreshInterval = setInterval(async () => {
      try {
        ...
        if (this.reconnecting) {
          console.log(
            `[Instance:${this.instanceId}] Skipping scheduled refresh - already reconnecting`
          );
          return;
        }

        console.log(
          `[Instance:${this.instanceId}] Running scheduled connection health check`
        );
        ...
        if (isConnectedNow && !isReallyConnected) {
          console.log(
            `[Instance:${this.instanceId}] Connection appears connected but isn't working, refreshing...`
          );
          ...
          this.lastRefreshTime = Date.now();
          console.log(
            `[Instance:${this.instanceId}] Connection refreshed successfully`
          );
        } else if (!isConnectedNow && !this.reconnecting) {
          // Try to reconnect if not connected at all and not already reconnecting
          console.log(
            `[Instance:${this.instanceId}] Not connected, attempting to reconnect...`
          );
          await this.connect(false);

          this.lastRefreshTime = Date.now();
          console.log(
            `[Instance:${this.instanceId}] Connection attempt completed`
          );
        } else if (isConnectedNow && isReallyConnected) {
          console.log(
            `[Instance:${this.instanceId}] Connection is working properly, no refresh needed`
          );
          this.lastRefreshTime = Date.now();
        }
      } catch (error) {
        console.error(
          `[Instance:${this.instanceId}] Error during scheduled connection refresh:`,
          error
        );
      }
  ...
  stopConnectionRefreshTimer() {
    if (this.connectionRefreshInterval) {
      ...
      this.connectionRefreshInterval = null;
      console.log(
        `[Instance:${this.instanceId}] Stopped connection refresh timer`
      );
    }
  }
  ...
  async verifyActiveConnection() {
    try {
      ...
      console.error(
        `[Instance:${this.instanceId}] Error verifying connection:`,
        error
      );
      return false;
    }
  ...
  async connect(showQrCode = false) {
    try {
      ...
      if (!this.serviceActive) {
        console.log(
          `[Instance:${this.instanceId}] Cannot connect - service is marked as inactive`
        );
        return false;
      }
      ...
      if (!this.clients || !this.clients.whatsAppClient) {
        console.log(
          `[Instance:${this.instanceId}] WhatsApp client not initialized, initializing now...`
        );
      ...
      if (
        ...
        console.log(
          `[Instance:${this.instanceId}] Set QR code display to: ${showQrCode}`
        );
      } else {
        console.warn(
          `[Instance:${this.instanceId}] Cannot set showQrCode flag, whatsAppClient.setShowQrCode is not a function`
        );
      }
      ...
      while (!success && retryCount < maxRetries) {
        try {
          console.log(
            `[Instance:${this.instanceId}] Initialization attempt ${
              retryCount + 1
            }/${maxRetries}`
          );
          ...
          if (
            ...
            console.error(
              `[Instance:${this.instanceId}] WhatsApp client not properly initialized or missing initialize method`
            );
            break;
          }
          ...
          if (success) {
            console.log(
              `[Instance:${this.instanceId}] WhatsApp client initialized successfully`
            );
            break;
          } else {
            console.log(
              `[Instance:${this.instanceId}] Initialization returned false, retrying...`
            );
          }
        } catch (initError) {
          console.error(
            `[Instance:${
              this.instanceId
            }] Error during initialization attempt ${retryCount + 1}:`,
            initError
          );
        }
  ...
  async attemptReconnect() {
    try {
      ...
      if (this.reconnecting) {
        console.log(
          `[Instance:${this.instanceId}] Reconnection already in progress`
        );
        return;
      }
      ...
      if (!this.serviceActive) {
        console.log(
          `[Instance:${this.instanceId}] Cannot reconnect - service is marked as inactive`
        );
      ...
      if (this.reconnectAttempts >= this.maxReconnectAttempts) {
        console.error(
          `[Instance:${this.instanceId}] Max reconnection attempts (${this.maxReconnectAttempts}) reached`
        );
        this.reconnecting = false;
        return;
      }
      ...
      console.log(
        `[Instance:${this.instanceId}] Attempting reconnect in ${backoffTime}ms (attempt ${this.reconnectAttempts}/${this.maxReconnectAttempts})`
      );
      ...
      console.log(
        `[Instance:${this.instanceId}] Executing reconnection attempt ${this.reconnectAttempts}`
      );
      ...
      if (success && this.isConnected()) {
        ...
        console.log(
          `[Instance:${this.instanceId}] Reconnection attempt failed, will retry`
        );
      ...
      console.error(
        `[Instance:${this.instanceId}] Error during reconnection:`,
        error
      );
      this.reconnecting = false;
  ...
  async ensureConnected() {
    try {
      ...
      if (this.isConnected()) {
        console.log(
          `[Instance:${this.instanceId}] Already connected, no action needed`
        );
        return true;
      }

      console.log(
        `[Instance:${this.instanceId}] Not connected, attempting to restore connection`
      );
      ...
      if (connected && this.isConnected()) {
        console.log(
          `[Instance:${this.instanceId}] Connection restored successfully`
        );
        return true;
      }
      ...
      if (
        this.clients.whatsAppClient &&
        (await this.clients.whatsAppClient.isAuthenticated())
      ) {
        console.log(
          `[Instance:${this.instanceId}] Client is pre-authenticated, but not yet connected`
        );
        ...
        try {
          ...
          if (this.isConnected()) {
            console.log(
              `[Instance:${this.instanceId}] Session restored successfully`
            );
            return true;
          }
        } catch (restoreError) {
          console.error(
            `[Instance:${this.instanceId}] Error restoring session:`,
            restoreError
          );
        }
      }
      ...
      console.log(
        `[Instance:${this.instanceId}] Could not restore connection automatically, QR code scan needed`
      );
      return false;
    } catch (error) {
      console.error(
        `[Instance:${this.instanceId}] Error ensuring connection:`,
        error
      );
      return false;
    }
  ...
  async disconnect(logOut = false) {
    try {
      ...
      try {
        ...
        console.log(
          `[Instance:${this.instanceId}] WhatsApp client disconnected successfully`
        );
      } catch (disconnectError) {
        console.error(
          `[Instance:${this.instanceId}] Error during client disconnect:`,
          disconnectError
        );
        // Continue anyway
      }
  ...
  async loadSettings() {
    try {
      ...
      if (fs.existsSync(settingsPath)) {
        ...
        console.log(
          `[Instance:${this.instanceId}] Loaded settings from instance-specific file`
        );
        return settings;
      } else {
        ...
        console.log(
          `[Instance:${this.instanceId}] Created new settings file with defaults`
        );
  ...
  async saveSettings(settings) {
    try {
      ...
      console.log(
        `[Instance:${this.instanceId}] Saved settings to instance file: ${settingsPath}`
      );
      return true;
    } catch (error) {
      console.error(
  ...
  isConnected() {
    ...
    if (typeof this.clients.whatsAppClient.isReady === "function") {
      try {
        ...
        console.error(
          `[Instance:${this.instanceId}] Error checking client ready state:`,
          error
        );
        return false;
      }
