...
class InteractionHandler {
  constructor() {
    ...
    console.log(
      "[InteractionHandler] Initialized with direct component access"
    );
  }
  ...
  initializeButtonHandlers() {
    try {
        ...
        if (item.isFile() && item.name.endsWith(".js")) {
          ...
          try {
            ...
            if (button && button.customId) {
              ...
              console.log(
                `[InteractionHandler] Registered button handler: ${button.customId}`
              );
            } else if (
              button &&
              (button.regex || typeof button.matches === "function")
            ) {
              this.regexButtonHandlers.push(button);
              console.log(
                `[InteractionHandler] Registered regex button handler from: ${item.name}`
              );
            }
          } catch (error) {
            console.error(
              `[InteractionHandler] Error loading button handler ${item.name}:`,
              error
            );
          }
          ...
          try {
            const subdirFiles = readdirSync(itemPath).filter((file) =>
              file.endsWith(".js")
            );
            ...
            for (const file of subdirFiles) {
              try {
                ...
                if (button && button.customId) {
                  this.buttonHandlers.set(button.customId, button);
                  console.log(
                    `[InteractionHandler] Registered button from subdirectory: ${button.customId}`
                  );
                } else if (
                  button &&
                  (button.regex || typeof button.matches === "function")
                  ...
                  this.regexButtonHandlers.push(button);
                  console.log(
                    `[InteractionHandler] Registered regex button from subdirectory: ${item.name}/${file}`
                  );
                }
              } catch (error) {
                console.error(
                  `[InteractionHandler] Error loading button from subdirectory ${item.name}/${file}:`,
                  error
                );
              }
            }
            ...
            console.error(
              `[InteractionHandler] Error processing subdirectory ${item.name}:`,
              subdirError
            );
          }
        }
      ...
      console.log(
        `[InteractionHandler] Loaded ${this.buttonHandlers.size} direct buttons and ${this.regexButtonHandlers.length} regex buttons`
      );
    } catch (error) {
      console.error(
        "[InteractionHandler] Error initializing button handlers:",
        error
      );
    }
  }
    ...
    try {
      ...
      const processDirectory = (directory, subpath = "") => {
        try {
            ...
            if (item.isFile() && item.name.endsWith(".js")) {
              ...
              try {
                ...
                if (modal && modal.customId) {
                  ...
                  console.log(
                    `[InteractionHandler] Registered modal handler: ${subpath}${item.name} with ID ${modal.customId}`
                  );
                } else if (
                  modal &&
                  (modal.regex || typeof modal.matches === "function")
                ) {
                  this.regexModalHandlers.push(modal);
                  console.log(
                    `[InteractionHandler] Registered regex modal handler from: ${subpath}${item.name}`
                  );
                }
              } catch (error) {
                console.error(
                  `[InteractionHandler] Error loading modal handler ${subpath}${item.name}:`,
                  error
                );
              }
            } else if (item.isDirectory()) {
          ...
          console.error(
            `[InteractionHandler] Error processing modals in ${directory}:`,
            error
          );
        }
      };
      ...
      console.log(
        `[InteractionHandler] Loaded ${this.modalHandlers.size} direct modals and ${this.regexModalHandlers.length} regex modals`
      );
    } catch (error) {
      console.error(
        "[InteractionHandler] Error initializing modal handlers:",
        error
      );
    }
  }
  ...
  /**
   * Find instance for an interaction without using InstanceManager directly
  ...
  getInstanceForInteraction(interaction) {
    ...
    try {
      console.log(
        `[InteractionHandler] Finding instance for guild ${interaction.guildId}`
      );

      // Special case for customize_messages_modal
      if (
        interaction.isModalSubmit() &&
        interaction.customId === "customize_messages_modal"
      ) {
        console.log(
          "[InteractionHandler] Using temporary instance for customize_messages_modal"
        );
        return {
          customSettings: {},
          isTemporary: true,
      ...
      // Look for instance in route map first
      if (interaction.client._instanceRoutes) {
        // Try by channel's parent category
        if (interaction.channel && interaction.channel.parentId) {
          const categoryId = interaction.channel.parentId;
          console.log(`[InteractionHandler] Channel parent ID: ${categoryId}`);

          if (interaction.client._instanceRoutes.has(categoryId)) {
            const route = interaction.client._instanceRoutes.get(categoryId);
            if (route && route.instance) {
              console.log(
                `[InteractionHandler] Found instance via category: ${
                  route.instance.instanceId || "unknown"
                }`
              );
        ...
        // Try all routes for guild match
        for (const [
          _,
          routeInfo,
        ] of interaction.client._instanceRoutes.entries()) {
          if (
            routeInfo &&
            routeInfo.instance &&
            routeInfo.instance.guildId === interaction.guildId
          ) {
            console.log(
              `[InteractionHandler] Found instance via guild match: ${
                routeInfo.instance.instanceId || "unknown"
              }`
            );
      ...
      // If we couldn't find from route map, try reading config directly
      try {
        const fs = require("fs");
        ...
        if (fs.existsSync(configPath)) {
          ...
          for (const [instanceId, config] of Object.entries(configs)) {
            if (config.guildId === interaction.guildId) {
              console.log(
                `[InteractionHandler] Created instance from config with ID: ${instanceId}`
              );
        ...
        console.error(
          `[InteractionHandler] Error reading instance config:`,
          configError
        );
      }

      // Create minimal temporary instance when all else fails
      console.log(
        `[InteractionHandler] Creating minimal temporary instance for guild ${interaction.guildId}`
      );
      return {
        instanceId: interaction.guildId,
        guildId: interaction.guildId,
      ...
      console.error(
        "[InteractionHandler] Error getting instance for interaction:",
        error
      );

      // Return minimal temporary instance on error for safety
      return {
        instanceId: interaction.guildId,
        guildId: interaction.guildId,
        customSettings: {},
  ...
  findButtonHandler(customId) {
    ...
    for (const handler of this.regexButtonHandlers) {
      if (
        (handler.regex && handler.regex.test(customId)) ||
  ...
  async handleInteraction(interaction) {
    try {
      ...
      if (interaction.isButton()) {
        console.log(
          `[InteractionHandler] Processing button: ${interaction.customId}`
        );
        ...
        if (handler && typeof handler.execute === "function") {
          ...
          console.log(
            `[InteractionHandler] Found handler for button ${interaction.customId}, executing...`
          );
          try {
            ...
            console.error(
              `[InteractionHandler] Error in button handler for ${interaction.customId}:`,
              buttonError
            );
          ...
          console.log(
            `[InteractionHandler] No handler found for button: ${interaction.customId}`
          );
          ...
          try {
            ...
            if (
              ButtonLoader &&
              typeof ButtonLoader.handleInteraction === "function"
            ) {
              instance = this.getInstanceForInteraction(interaction);
              const handled = await ButtonLoader.handleInteraction(
                interaction,
                instance
              );
            ...
            console.error(
              `[InteractionHandler] Error using ButtonLoader fallback:`,
              buttonLoaderError
            );
          }
        }
      ...
      if (interaction.isModalSubmit()) {
        console.log(
          `[InteractionHandler] Processing modal submission: ${interaction.customId}`
        );
        ...
        if (handler && typeof handler.execute === "function") {
          ...
          try {
            ...
            console.error(
              `[InteractionHandler] Error in modal handler for ${interaction.customId}:`,
              modalError
            );
          ...
          console.log(
            `[InteractionHandler] No direct handler found for modal: ${interaction.customId}`
          );
          ...
          try {
            ...
            if (
              ModalLoader &&
              typeof ModalLoader.handleInteraction === "function"
            ) {
              instance = this.getInstanceForInteraction(interaction);
              const handled = await ModalLoader.handleInteraction(
                interaction,
                instance
              );
              if (handled) return true;
            ...
            console.error(
              `[InteractionHandler] Error using ModalLoader fallback:`,
              modalLoaderError
            );
          }
        }
      ...
      if (interaction.isCommand()) {
        console.log(
          `[InteractionHandler] Processing command: ${interaction.commandName}`
        );

        // Look for command in client.commands first (from registerHandlers.js)
        const clientCommand = interaction.client.commands?.get(
        ...
        if (clientCommand && typeof clientCommand.execute === "function") {
          ...
          try {
            ...
            console.error(
              `[InteractionHandler] Error in command handler for ${interaction.commandName}:`,
              commandError
            );
          ...
          console.log(
            `[InteractionHandler] Command not found in client.commands: ${interaction.commandName}`
          );
        }

        // Try ModuleLoader as fallback
        const moduleCommand = this.moduleLoader.getCommand(
          interaction.commandName
        );
        if (moduleCommand && typeof moduleCommand.execute === "function") {
          ...
          try {
            ...
            console.error(
              `[InteractionHandler] Error in module command for ${interaction.commandName}:`,
              moduleCommandError
            );
          ...
          console.log(
            `[InteractionHandler] Command not found in ModuleLoader: ${interaction.commandName}`
          );
        }
      }
      ...
      if (interaction.isStringSelectMenu()) {
        console.log(
          `[InteractionHandler] Processing select menu: ${interaction.customId}`
        );
        ...
        try {
          ...
          if (
            SelectMenuLoader &&
            typeof SelectMenuLoader.handleInteraction === "function"
          ) {
            instance = this.getInstanceForInteraction(interaction);
            const handled = await SelectMenuLoader.handleInteraction(
              interaction,
              instance
            );
            if (handled) return true;
          ...
          console.error(
            `[InteractionHandler] Error with select menu loader:`,
            selectMenuError
          );
        }
      }
      ...
      console.log(
        `[InteractionHandler] No handler found for interaction type ${
          interaction.type
      ...
      console.error(
        "[InteractionHandler] Error in interaction handler:",
        error
      );
      ...
      try {
        ...
        console.error(
          "[InteractionHandler] Error sending error response:",
          replyError
        );
      }
  ...
  async handleMessage(message) {
    try {
      ...
      if (
        message.client._instanceRoutes &&
        message.client._instanceRoutes.has(categoryId)
      ) {
        ...
        if (handler && typeof handler.handleDiscordMessage === "function") {
          try {
            ...
            console.error(
              `[InteractionHandler] Error in message handler for ${routeInfo.instanceId}:`,
              handlerError
            );
          }
        }
        ...
        if (instance?.handlers?.discordHandler?.handleDiscordMessage) {
          try {
            await instance.handlers.discordHandler.handleDiscordMessage(
              message
            );
            return true;
          } catch (instanceError) {
            console.error(
              `[InteractionHandler] Error in instance message handler for ${instance.instanceId}:`,
              instanceError
            );
          }
        }
      ...
      if (message.client._instanceRoutes) {
        for (const [_, routeInfo] of message.client._instanceRoutes.entries()) {
          if (
            routeInfo.instance &&
            routeInfo.instance.guildId === message.guild.id
          ) {
            ...
            if (instance?.handlers?.discordHandler?.handleDiscordMessage) {
              try {
                ...
                console.error(
                  `[InteractionHandler] Error in fallback message handler for ${instance.instanceId}:`,
                  fallbackError
                );
              }
            }
      ...
      console.error(
        "[InteractionHandler] Error handling Discord message:",
        error
      );
      return false;
    }
