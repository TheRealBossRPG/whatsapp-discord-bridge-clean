...
class BaileysClient extends EventEmitter {
  ...
  constructor(options = {}) {
    ...
    this.authFolder = options.authFolder || path.join(__dirname, '..', '..', 'instances', 'default', 'auth');
  ...
  cleanTempDirectory() {
    try {
      if (fs.existsSync(this.tempDir)) {
            ...
            if (stats.isFile()) {
              ...
              try {
                ...
                if (e.code !== 'EBUSY' && e.code !== 'EPERM') {
                  console.error(`[BaileysClient:${this.instanceId}] Error deleting file ${file}:`, e);
                }
              }
            }
          } catch (statError) {
        ...
        if (deletedCount > 0) {
          console.log(`[BaileysClient:${this.instanceId}] Cleaned ${deletedCount} old files from temp directory`);
        }
      }
    } catch (error) {
      console.error(`[BaileysClient:${this.instanceId}] Error cleaning temp directory:`, error);
    }
  }
  ...
  formatPhoneNumber(phoneNumber) {
    ...
    formatted = formatted.replace(/@s\.whatsapp\.net/g, '')
                        .replace(/@c\.us/g, '')
                        .replace(/@g\.us/g, '')
                        .replace(/@broadcast/g, '')
                        .replace(/@.*$/g, '');
  ...
  extractPhoneNumber(jid) {
    ...
    try {
      ...
      console.error(`[BaileysClient:${this.instanceId}] Error extracting phone number:`, error);
      return jid.split('@')[0];
    }
  }
  ...
  async initialize() {
    try {
      console.log(`[BaileysClient:${this.instanceId}] Initializing WhatsApp client...`);
      
      // Check if we're already initialized
      if (this.sock) {
        console.log(`[BaileysClient:${this.instanceId}] Client already initialized`);
        return true;
      }
        ...
        for (const credsFile of credsFiles) {
          if (fs.existsSync(credsFile)) {
            try {
              ...
              console.log(`[BaileysClient:${this.instanceId}] Deleted existing creds file: ${credsFile}`);
            } catch (e) {
              console.error(`[BaileysClient:${this.instanceId}] Error deleting creds file: ${e.message}`);
            }
          }
        }
      }
      ...
      if (error instanceof Boom && error.output?.statusCode === 429) {
        if (this.connectionRetryCount < this.maxRetries) {
          ...
          console.log(`[BaileysClient:${this.instanceId}] Rate limited, retrying in 10 seconds... (${this.connectionRetryCount}/${this.maxRetries})`);
  ...
  setupEventHandlers() {
    if (!this.sock) {
      console.error(`[BaileysClient:${this.instanceId}] Cannot set up event handlers: Socket not initialized`);
      return;
    }
    ...
    this.sock.ev.on('connection.update', (update) => {
      ...
      if (qr) {
        console.log(`[BaileysClient:${this.instanceId}] New QR code generated (attempt ${this.qrRetryCount + 1}/${this.maxRetries})`);
        ...
        setTimeout(() => {
          if (!this.isReady && this.qrRetryCount === this.maxRetries) {
            console.log(`[BaileysClient:${this.instanceId}] QR code timed out after ${this.maxRetries} attempts`);
      ...
      if (connection === 'close') {
        ...
        console.log(`[BaileysClient:${this.instanceId}] Connection closed with status: ${statusCode}, reconnect: ${shouldReconnect}`);
        ...
        if (shouldReconnect && this.connectionRetryCount < this.maxRetries) {
          this.connectionRetryCount++;
          console.log(`[BaileysClient:${this.instanceId}] Attempting to reconnect... (${this.connectionRetryCount}/${this.maxRetries})`);
        ...
        console.log(`[BaileysClient:${this.instanceId}] Connection opened successfully`);
      ...
      try {
        ...
        for (const msg of m.messages) {
          ...
          try {
            ...
            console.log(`[BaileysClient:${this.instanceId}] Received message from ${jid}`, {
            ...
            if (msg.message?.conversation) {
              ...
              console.log(`[BaileysClient:${this.instanceId}] Unknown message type from ${jid}`, {
            ...
            console.error(`[BaileysClient:${this.instanceId}] Error processing individual message:`, msgError);
            // Continue to next message even if one fails
          }
        }
      } catch (error) {
        console.error(`[BaileysClient:${this.instanceId}] Error in messages.upsert handler:`, error);
      }
    });
    ...
    this.sock.ev.on('messages.update', (updates) => {
      try {
        ...
        console.error(`[BaileysClient:${this.instanceId}] Error in messages.update handler:`, error);
      }
    });
    ...
    this.sock.ev.on('group-participants.update', (update) => {
      try {
        ...
        console.error(`[BaileysClient:${this.instanceId}] Error in group-participants.update handler:`, error);
      }
    });
  }
  ...
  async sendTextMessage(to, text) {
    try {
      ...
      // Send message
      console.log(`[BaileysClient:${this.instanceId}] Sending text message to ${recipient}`);
  ...
  async sendMediaMessage(to, mediaPath, caption = '', mediaType = 'image') {
    try {
      ...
      // Send message
      console.log(`[BaileysClient:${this.instanceId}] Sending ${mediaType} message to ${recipient}`);
      const result = await this.sock.sendMessage(recipient, messageContent);
      
      return result;
    } catch (error) {
      console.error(`[BaileysClient:${this.instanceId}] Error sending media message:`, error);
      throw error;
    }
  }
  ...
  async getProfilePicture(jid) {
    try {
      ...
      try {
        ...
        console.log(`[BaileysClient:${this.instanceId}] No profile picture for ${formattedJid}`);
        return null;
      }
    } catch (error) {
  ...
  async disconnect(logout = false) {
    try {
      ...
      // Logout if requested (this will delete the auth files)
      if (logout) {
        try {
          await this.sock.logout();
          console.log(`[BaileysClient:${this.instanceId}] Logged out successfully`);
        } catch (error) {
          console.error(`[BaileysClient:${this.instanceId}] Error logging out:`, error);
        }
      }
      ...
      // Force clear socket reference on error
      this.sock = null;
      this.isReady = false;
