// modules/BridgeInstanceManager.js - Simplified to work directly with DiscordServerInstance
const fs = require('fs');
const path = require('path');
const DiscordServerInstance = require('./DiscordServerInstance');

class BridgeInstanceManager {
  constructor() {
    this.instances = new Map(); // Map of instance ID to instance object
    ...
    try {
      ...
      for (const [instanceId, config] of Object.entries(this.configs)) {
        ...
        serializable[instanceId] = {
          ...
          transcriptChannelId: String(config.transcriptChannelId),
  ...
  async createInstance(config) {
    try {
      ...
      const instanceId = config.guildId;
      
      console.log(`Creating instance ${instanceId} for guild ${config.guildId}...`);
      ...
      // Create paths for this instance
      const paths = {
        auth: path.join(instanceDir, 'auth'),
        temp: path.join(instanceDir, 'temp'),
        transcripts: path.join(instanceDir, 'transcripts'),
        assets: path.join(instanceDir, 'assets')
      };
      ...
      // Create a new DiscordServerInstance directly
      const instance = new DiscordServerInstance({
        instanceId: instanceId,
        discordClient: config.discordClient,
        guildId: config.guildId,
        categoryId: config.categoryId,
        transcriptChannelId: config.transcriptChannelId || null,  // Make it optional
        vouchChannelId: config.vouchChannelId || null,  // Make it optional
        paths: paths,
      ...
      console.error(`Error creating instance for guild ${config.guildId}:`, error);
      throw error;
    }
  }
      ...
      for (const [instanceId, config] of Object.entries(this.configs)) {
        try {
          ...
          if (!instance) {
            ...
            instance = await this.createInstance(initialConfig);
          }
  ...
  async saveInstanceSettings(instanceId, settings) {
    try {
      ...
      if (this.configs && this.configs[instanceId]) {
        ...
        // Try finding in configs by guildId
        const configEntry = Object.entries(this.configs).find(([_, config]) => config.guildId === instanceId);
        if (configEntry) {
          const [configId, _] = configEntry;
          this.configs[configId].customSettings = {
            ...this.configs[configId].customSettings,
            ...settings
          };
          ...
          // CRITICAL FIX: Also update direct channel properties
          if (settings.transcriptChannelId) {
            this.configs[configId].transcriptChannelId = settings.transcriptChannelId;
          }
          if (settings.vouchChannelId) {
            this.configs[configId].vouchChannelId = settings.vouchChannelId;
          }
...
module.exports = new BridgeInstanceManager();